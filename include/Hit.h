...
class Hit {
	...
	public:
	Hit() : TubeId(0), Time(0), Charge(0){}
	Hit(int thetubeid, double thetime, double thecharge) : TubeId(thetubeid), Time(thetime), Charge(thecharge){}
	virtual ~Hit(){};
...
class MCHit : public Hit {
	...
	public:
	MCHit() : Hit(), Parents(std::vector<int>{}) {}
	MCHit(int tubeid, double thetime, double thecharge, std::vector<int> theparents) : Hit(tubeid, thetime, thecharge), Parents(theparents) {}
	virtual ~MCHit(){};
